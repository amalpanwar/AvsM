...
On GitHub, the `Sync Premier League Results` workflow uses the repository secret named `FOOTBALL_DATA_API_TOKEN`. The workflow runs every 30 minutes, writes finished Premier League scores into `data/api-results.json`, regenerates `generated-data.js`, and pushes the update back to this repo. It uses football-data.org first and Native Stats as a fallback when Native Stats has already posted finals. The Excel workbook remains the fixture source and does not need to be modified for API settlement.
